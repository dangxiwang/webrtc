...
std::string GetAddressFamilyAsString(
    rtclog::IceCandidatePairConfig::AddressFamily family) {
  switch (family) {
...
std::string GetNetworkTypeAsString(
    rtclog::IceCandidatePairConfig::NetworkType type) {
  switch (type) {
...
std::string GetCandidatePairLogDescriptionAsString(
  ...
  if (config.local_candidate_type == rtclog::IceCandidatePairConfig::RELAY) {
    local_candidate_type +=
        "(" + GetProtocolAsString(config.local_relay_protocol) + ")";
  }
  if (config.remote_candidate_type == rtclog::IceCandidatePairConfig::RELAY) {
    remote_candidate_type +=
        "(" + GetProtocolAsString(config.remote_relay_protocol) + ")";
  }
  ss << local_candidate_type << ":"
     ...
     << GetAddressFamilyAsString(config.candidate_pair_address_family);
...
void EventLogAnalyzer::CreateIceCandidatePairConfigGraph(Plot* plot) {
  ...
  for (const auto& config : ice_candidate_pair_configs_) {
    ...
    configs_by_cp_id[config.candidate_pair_id].label =
        GetCandidatePairLogDescriptionAsString(config);
  }
