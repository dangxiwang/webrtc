...
const char kContentNameAudio[] = "audio";
const char kContentNameVideo[] = "video";
const char kContentnameData[] = "data";
...
std::string GetContentNameAsString(
    rtclog::IceCandidatePairEvent::ContentName content_name) {
  switch (content_name) {
    case rtclog::IceCandidatePairEvent::AUDIO:
      return kContentNameAudio;
    case rtclog::IceCandidatePairEvent::VIDEO:
      return kContentNameVideo;
    case rtclog::IceCandidatePairEvent::DATA:
      return kContentnameData;
    default:
...
std::string GetIceCandidateTypeAsString(
    rtclog::IceCandidatePairEvent::IceCandidateType type) {
  switch (type) {
    case rtclog::IceCandidatePairEvent::LOCAL:
      return kIceCandidateTypeLocal;
    case rtclog::IceCandidatePairEvent::STUN:
      return kIceCandidateTypeStun;
    case rtclog::IceCandidatePairEvent::PRFLX:
      return kIceCandidateTypePrflx;
    case rtclog::IceCandidatePairEvent::RELAY:
...
std::string GetProtocolAsString(
    rtclog::IceCandidatePairEvent::Protocol protocol) {
  switch (protocol) {
    case rtclog::IceCandidatePairEvent::UDP:
      return kProtocolUdp;
    case rtclog::IceCandidatePairEvent::TCP:
      return kProtocolTcp;
    case rtclog::IceCandidatePairEvent::SSLTCP:
      return kProtocolSsltcp;
    case rtclog::IceCandidatePairEvent::TLS:
      return kProtocolTls;
    default:
      return kUnknownEnumValue;
  }
...
std::string GetAddressFamilyAsString(
    rtclog::IceCandidatePairEvent::AddressFamily family) {
  switch (family) {
    case rtclog::IceCandidatePairEvent::IPV4:
      return kAddressFamilyIpv4;
    case rtclog::IceCandidatePairEvent::IPV6:
...
std::string GetNetworkTypeAsString(
    rtclog::IceCandidatePairEvent::NetworkType type) {
  switch (type) {
    case rtclog::IceCandidatePairEvent::ETHERNET:
      return kNetworkTypeEthernet;
    case rtclog::IceCandidatePairEvent::LOOPBACK:
      return kNetworkTypeLoopback;
    case rtclog::IceCandidatePairEvent::WIFI:
      return kNetworkTypeWifi;
    case rtclog::IceCandidatePairEvent::VPN:
      return kNetworkTypeVpn;
    case rtclog::IceCandidatePairEvent::CELLULAR:
...
std::string GetCandidatePairLogDescriptionAsString(
    const ParsedRtcEventLog::IceCandidatePairEvent& event) {
  std::stringstream ss;
  ss << GetIceCandidateTypeAsString(event.local_candidate_type.value()) << ":"
     << GetNetworkTypeAsString(event.local_network_type.value()) << "->"
     << GetIceCandidateTypeAsString(event.remote_candidate_type.value()) << "@"
     << GetContentNameAsString(event.content_name.value()) << ":"
     << GetProtocolAsString(event.candidate_pair_protocol.value()) << ":"
     << GetAddressFamilyAsString(event.candidate_pair_address_family.value());
  return ss.str();
}
...
void EventLogAnalyzer::CreateIceConnectivityCheckGraph(Plot* plot) {
  ...
  for (const auto& cp_event : ice_candidate_pair_events_) {
    if (checks_by_cp_id.find(cp_event.candidate_pair_id) ==
        checks_by_cp_id.end()) {
      checks_by_cp_id[cp_event.candidate_pair_id] = TimeSeries(
          std::to_string(cp_event.candidate_pair_id),  // Placeholder label if
                                                       // the first parsed event
                                                       // for a candidate pair
                                                       // is not of type ADDED.
          LineStyle::kNone, PointStyle::kHighlight);
    }
    float x = ToCallTime(cp_event.timestamp);
    float y = static_cast<float>(cp_event.type);
    checks_by_cp_id[cp_event.candidate_pair_id].points.emplace_back(x, y);
    if (cp_event.type == rtclog::IceCandidatePairEvent::ADDED) {
      // TODO(qingsi): Implement the string getter of event description.
      checks_by_cp_id[cp_event.candidate_pair_id].label =
          GetCandidatePairLogDescriptionAsString(cp_event);
    }
  }
