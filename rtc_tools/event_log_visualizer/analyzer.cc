...
std::string GetCandidatePairDescriptionAsString(
    const ParsedRtcEventLog::IceCandidatePairEvent& event) {
  const auto& desc = event.candidate_pair_desc.value();
  std::stringstream ss;
  ss << desc.media_content() << ":" << desc.local_candidate_type() << ":"
     << desc.local_candidate_protocol() << ":" << desc.local_transport_address()
     << ":" << desc.local_network_type() << ":" + desc.remote_candidate_type()
     << ":" << desc.remote_candidate_protocol() << ":"
     << desc.remote_transport_address();
  return ss.str();
}
  ...
  for (const auto& cp_event : ice_candidate_pair_events_) {
    ...
    if (cp_event.type == rtclog::IceCandidatePairEvent::ADDED) {
      checks_by_cp_id[cp_event.candidate_pair_id].label =
          GetCandidatePairDescriptionAsString(cp_event);
    }
  }
