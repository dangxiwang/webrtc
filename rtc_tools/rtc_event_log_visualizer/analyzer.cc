...
RtpPacketReceived RtpPacketForBWEFromHeader(const RTPHeader& header) {
  ...
  // Set only field that might be relevant to the bandwidth estimatior.
  ...
  // Original header could have more extensions, there could be csrcs that are
...
void EventLogAnalyzer::CreateReceiveSideBweSimulationGraph(Plot* plot) {
  ...
  for (const auto& kv : incoming_rtp) {
    ...
    acked_bitrate.Update(packet.rtp.total_length, /*Should subtract header?*/
                         arrival_time_ms);
