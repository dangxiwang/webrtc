...
Call::~Call() {
  ...
  RTC_DCHECK(!transport_send_->GetWorkerQueue()->IsCurrent());
...
void Call::SetBitrateAllocationStrategy(
  ...
  // The transport_send_ unique_ptr has shorter life time than the task queue.
  // This check ensures that we are not accessing transport_send_ from the task
  // queue.
  RTC_CHECK(!transport_send_->GetWorkerQueue()->IsCurrent());
  transport_send_->GetWorkerQueue()->PostTask(Functor{
