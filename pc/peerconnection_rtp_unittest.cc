...
class OnSuccessObserver
 ...
 public:
  ...
  // webrtc::SetRemoteDescriptionObserver implementation.
  void OnSuccess() override { on_success_(); }
  void OnFailure(webrtc::FailureReason reason) override { RTC_NOTREACHED(); }
...
TEST_F(PeerConnectionRtpCallbacksTest, AddTrackWithoutStreamFiresOnAddTrack) {
  ...
  ASSERT_TRUE(callee->SetRemoteDescription(
      ...
      static_cast<webrtc::FailureReason*>(nullptr)));
...
// These tests examine the state of the peer connection's as a result of
...
TEST_F(PeerConnectionRtpObserverTest,
  ...
  auto srd1_callback = [&srd1_callback_called, &pc]() {
    EXPECT_EQ(1u, pc->GetReceivers().size());
