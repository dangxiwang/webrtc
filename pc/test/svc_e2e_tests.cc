...
TEST_P(SvcSelectLayerTest, CheckDecodeSelectedLayer) {
  ...
  constexpr int kInputHeight = 720;
  auto fixture = CreateTestFixture(
      UnitTest::GetInstance()->current_test_info()->name(),
      *network_emulation_manager->time_controller(),
      CreateTwoNetworkLinks(network_emulation_manager.get(),
      ...
      [this, simulcast_config, scalability_mode_str](PeerConfigurer* alice) {
        VideoConfig video(/*stream_label=*/"alice-video", /*width=*/1280,
                          /*height=*/720, /*fps=*/30);
        ...
        alice->AddVideoConfig(
            std::move(video),
            test::CreateFromYuvFileFrameGenerator(
                {test::ResourcePath("FourPeople_1280x720_30", "yuv")},
                kInputWidth, kInputHeight, 1));
        alice->SetVideoCodecs({video_codec_config});
      },
      [](PeerConfigurer* bob) {}, std::move(analyzer));
  fixture->Run(RunParams(TimeDelta::Seconds(5)));
  ...
  } else if (*resolution_ratio == ScalabilityModeResolutionRatio::kTwo) {
  ...
  } else if (*resolution_ratio == ScalabilityModeResolutionRatio::kOneHalf) {
