...
PeerConnection::CreateDataChannel(
  ...
  NoteUsageEvent(UsageEvent::DATA_CHANNEL_ADDED);
...
void PeerConnection::NoteUsageEvent(UsageEvent event) {
  RTC_DCHECK_RUN_ON(signaling_thread());
  usage_event_accumulator_.insert(event);
}

void PeerConnection::ReportUsagePattern() const {
  int signature = 0;
  for (const auto it : usage_event_accumulator_) {
    signature |= 1 << static_cast<int>(it);
  }
  // Compile-time check that the accumulator can accomodate all defined flags.
  RTC_DCHECK(1 << static_cast<int>(UsageEvent::USAGE_EVENT_MAX) <
             std::numeric_limits<int>::max());
  RTC_LOG(LS_ERROR) << "DEBUG: Usage signature is " << signature;
  if (uma_observer_) {
