...
// TODO(hbos): This is a test for api/setremotedescriptionobserver.h and should
// be in api/ instead of pc/, but the dependency on
...
TEST_F(SetRemoteDescriptionSessionObserverWrapperTest, OnCompleteWithSuccess) {
  observer_->OnComplete(webrtc::RTCError::OK());
  EXPECT_TRUE_WAIT(set_desc_observer_->called(), kDefaultTimeoutMs);
  EXPECT_TRUE(set_desc_observer_->result());
}

TEST_F(SetRemoteDescriptionSessionObserverWrapperTest, OnCompleteWithFailure) {
  observer_->OnComplete(webrtc::RTCError(
