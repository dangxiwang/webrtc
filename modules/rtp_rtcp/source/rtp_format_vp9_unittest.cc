...
void ParseAndCheckPacket(const uint8_t* packet,
  ...
  EXPECT_EQ(kVideoCodecVP9, parsed.video.codec);
  VerifyHeader(expected, parsed.video.codecHeader.VP9);
...
TEST_F(RtpDepacketizerVp9Test, ParseResolution) {
  ...
  RtpDepacketizer::ParsedPayload parsed;
  ASSERT_TRUE(depacketizer_->Parse(&parsed, packet, sizeof(packet)));
  EXPECT_EQ(kWidth[0], parsed.video.width);
  EXPECT_EQ(kHeight[0], parsed.video.height);
}
