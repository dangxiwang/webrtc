...
bool RtpDepacketizerH264::Parse(ParsedPayload* parsed_payload,
  ...
  parsed_payload->video.codecHeader.H264.nalus_length = 0;
...
bool RtpDepacketizerH264::ProcessStapAOrSingleNalu(
  ...
  parsed_payload->video.codec = kVideoCodecH264;
  parsed_payload->video.is_first_packet_in_frame = true;
  RTPVideoHeaderH264* h264_header = &parsed_payload->video.codecHeader.H264;
  ...
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    ...
    switch (nalu.type) {
      case H264::NaluType::kSps: {
        ...
        if (sps) {
          parsed_payload->video.width = sps->width;
          parsed_payload->video.height = sps->height;
          nalu.sps_id = sps->id;
        } else {
    ...
    RTPVideoHeaderH264* h264 = &parsed_payload->video.codecHeader.H264;
...
bool RtpDepacketizerH264::ParseFuaNalu(
  ...
  parsed_payload->video.height = 0;
  parsed_payload->video.codec = kVideoCodecH264;
  parsed_payload->video.is_first_packet_in_frame = first_fragment;
  RTPVideoHeaderH264* h264 = &parsed_payload->video.codecHeader.H264;
