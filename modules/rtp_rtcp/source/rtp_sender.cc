...
void RTPSender::UpdateDelayStatistics(int64_t capture_time_ms,
  ...
  uint64_t total_packet_send_delay_ms = 0;
  {
    ...
    int64_t avg_ms = DivideRoundToNearest(sum_delays_ms_, send_delays_.size());
    RTC_DCHECK_GE(avg_ms, 0);
    RTC_DCHECK_LE(avg_ms, std::numeric_limits<int>::max());
    avg_delay_ms = rtc::dchecked_cast<int>(avg_ms);
  }
