...
bool AbsoluteCaptureTimeSender::ShouldSendExtension(
  ...
  // Tecnically right conversion is to take the error in seconds and multiple it
  // by 2^32, but such conversion and multiplication are swapped to avoid using
  // float numbers and still have good precision.
  static constexpr int64_t kInterpolationMaxErrorx32 =
      (kInterpolationMaxError * (uint64_t{1} << 32)).seconds();
  ...
  const uint64_t absolute_capture_timestamp =
      static_cast<uint64_t>(absolute_capture_time);
  // Value larger than max(uint64_t) / 2 represents negative delta, so
  // taking std::min would return absolute value of the delta.
  const int64_t interpolation_error = std::min(
  ...
  if (interpolation_error > kInterpolationMaxErrorx32) {
    return true;
  }
