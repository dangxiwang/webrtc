...
int ParseVP8FrameSize(RtpDepacketizer::ParsedPayload* parsed_payload,
  ...
  parsed_payload->video.width = ((data[7] << 8) + data[6]) & 0x3FFF;
  parsed_payload->video.height = ((data[9] << 8) + data[8]) & 0x3FFF;
  return 0;
}
...
bool RtpDepacketizerVp8::Parse(ParsedPayload* parsed_payload,
  ...
  parsed_payload->video.width = 0;
  parsed_payload->video.height = 0;
  parsed_payload->video.is_first_packet_in_frame =
      beginning_of_partition && (partition_id == 0);
  parsed_payload->video.simulcastIdx = 0;
  parsed_payload->video.codec = kVideoCodecVP8;
  parsed_payload->video.codecHeader.VP8.nonReference =
      (*payload_data & 0x20) ? true : false;  // N bit
  parsed_payload->video.codecHeader.VP8.partitionId = partition_id;
  parsed_payload->video.codecHeader.VP8.beginningOfPartition =
      beginning_of_partition;
  parsed_payload->video.codecHeader.VP8.pictureId = kNoPictureId;
  parsed_payload->video.codecHeader.VP8.tl0PicIdx = kNoTl0PicIdx;
  parsed_payload->video.codecHeader.VP8.temporalIdx = kNoTemporalIdx;
  parsed_payload->video.codecHeader.VP8.layerSync = false;
  parsed_payload->video.codecHeader.VP8.keyIdx = kNoKeyIdx;
  ...
  if (extension) {
    const int parsed_bytes =
        ParseVP8Extension(&parsed_payload->video.codecHeader.VP8, payload_data,
                          payload_data_length);
    if (parsed_bytes < 0)
      return false;
