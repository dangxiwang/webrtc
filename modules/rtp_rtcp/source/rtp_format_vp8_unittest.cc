...
TEST_F(RtpDepacketizerVp8Test, BasicHeader) {
  ...
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 0, 1, 4);
  VerifyExtensions(&payload.video, kNoPictureId, kNoTl0PicIdx, kNoTemporalIdx,
                   kNoKeyIdx);
}
...
TEST_F(RtpDepacketizerVp8Test, PictureID) {
  ...
  EXPECT_EQ(kVideoFrameDelta, payload.frame_type);
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 1, 0, 0);
  VerifyExtensions(&payload.video, kPictureId, kNoTl0PicIdx, kNoTemporalIdx,
                   kNoKeyIdx);
  ...
  VerifyBasicHeader(&payload.video, 1, 0, 0);
  VerifyExtensions(&payload.video, (kPictureId << 8) + kPictureId, kNoTl0PicIdx,
...
TEST_F(RtpDepacketizerVp8Test, Tl0PicIdx) {
  ...
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 0, 1, 0);
  VerifyExtensions(&payload.video, kNoPictureId, kTl0PicIdx, kNoTemporalIdx,
                   kNoKeyIdx);
}
...
TEST_F(RtpDepacketizerVp8Test, TIDAndLayerSync) {
  ...
  EXPECT_EQ(kVideoFrameDelta, payload.frame_type);
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 0, 0, 8);
  VerifyExtensions(&payload.video, kNoPictureId, kNoTl0PicIdx, 2, kNoKeyIdx);
  EXPECT_FALSE(payload.video.codecHeader.VP8.layerSync);
}
...
TEST_F(RtpDepacketizerVp8Test, MultipleExtensions) {
  ...
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 0, 0, 8);
  VerifyExtensions(&payload.video, (17 << 8) + 17, 42, 1, 17);
}
...
TEST_F(RtpDepacketizerVp8Test, TestWithPacketizer) {
  ...
  EXPECT_EQ(kVideoFrameKey, payload.frame_type);
  EXPECT_EQ(kVideoCodecVP8, payload.video.codec);
  VerifyBasicHeader(&payload.video, 1, 1, 0);
  VerifyExtensions(&payload.video, input_header.pictureId,
                   input_header.tl0PicIdx, input_header.temporalIdx,
                   input_header.keyIdx);
  EXPECT_EQ(payload.video.codecHeader.VP8.layerSync, input_header.layerSync);
}
