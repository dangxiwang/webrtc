...
TEST_F(RtpDepacketizerH264Test, TestSingleNalu) {
  ...
  EXPECT_EQ(kVideoCodecH264, payload.video.codec);
  EXPECT_TRUE(payload.video.is_first_packet_in_frame);
  EXPECT_EQ(kH264SingleNalu, payload.video.codecHeader.H264.packetization_type);
  EXPECT_EQ(kIdr, payload.video.codecHeader.H264.nalu_type);
}
...
TEST_F(RtpDepacketizerH264Test, TestSingleNaluSpsWithResolution) {
  ...
  EXPECT_TRUE(payload.video.is_first_packet_in_frame);
  EXPECT_EQ(kH264SingleNalu, payload.video.codecHeader.H264.packetization_type);
  EXPECT_EQ(1280u, payload.video.width);
  EXPECT_EQ(720u, payload.video.height);
}
...
TEST_F(RtpDepacketizerH264Test, TestStapAKey) {
  ...
  EXPECT_EQ(kVideoCodecH264, payload.video.codec);
  EXPECT_TRUE(payload.video.is_first_packet_in_frame);
  const RTPVideoHeaderH264& h264 = payload.video.codecHeader.H264;
...
TEST_F(RtpDepacketizerH264Test, TestStapANaluSpsWithResolution) {
  ...
  EXPECT_EQ(kVideoCodecH264, payload.video.codec);
  EXPECT_TRUE(payload.video.is_first_packet_in_frame);
  EXPECT_EQ(kH264StapA, payload.video.codecHeader.H264.packetization_type);
  EXPECT_EQ(1280u, payload.video.width);
  EXPECT_EQ(720u, payload.video.height);
}
...
TEST_F(RtpDepacketizerH264Test, TestStapADelta) {
  ...
  EXPECT_EQ(kH264StapA, payload.video.codecHeader.H264.packetization_type);
  // NALU type for aggregated packets is the type of the first packet only.
  EXPECT_EQ(kSlice, payload.video.codecHeader.H264.nalu_type);
}
...
TEST_F(RtpDepacketizerH264Test, TestFuA) {
  ...
  EXPECT_EQ(kVideoFrameKey, payload.frame_type);
  EXPECT_EQ(kVideoCodecH264, payload.video.codec);
  EXPECT_FALSE(payload.video.is_first_packet_in_frame);
  {
    const RTPVideoHeaderH264& h264 = payload.video.codecHeader.H264;
    EXPECT_EQ(kH264FuA, h264.packetization_type);
    EXPECT_EQ(kIdr, h264.nalu_type);
