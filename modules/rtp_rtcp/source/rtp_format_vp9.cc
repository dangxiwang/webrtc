...
bool RtpDepacketizerVp9::Parse(ParsedPayload* parsed_payload,
  ...
  parsed_payload->video.codec = kVideoCodecVP9;

  parsed_payload->frame_type = p_bit ? kVideoFrameDelta : kVideoFrameKey;

  RTPVideoHeaderVP9* vp9 = &parsed_payload->video.codecHeader.VP9;
  ...
  if (v_bit) {
    ...
    if (vp9->spatial_layer_resolution_present) {
      ...
      parsed_payload->video.width = vp9->width[0];
      parsed_payload->video.height = vp9->height[0];
    }
  }
