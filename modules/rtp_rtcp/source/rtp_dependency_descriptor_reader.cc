...
uint32_t RtpDependencyDescriptorReader::ReadBits(size_t bit_count) {
  // Assume most of the time buffer has enough bits and thus do not bother
  // to stop parsing as soon as it failed.
  uint32_t value = 0;
...
std::unique_ptr<FrameDependencyStructure>
RtpDependencyDescriptorReader::ReadTemplateDependencyStructure() {
  auto structure = absl::make_unique<FrameDependencyStructure>();
  structure->structure_id = ReadBits(6);
  structure->num_decode_targets = ReadBits(5) + 1;

  structure->templates = ReadTemplateLayers();
  ReadTemplateDtis(structure.get());
  ReadTemplateFdiffs(structure.get());
  ReadTemplateChains(structure.get());

  uint32_t has_resolutions = ReadBits(1);
  if (has_resolutions)
    ReadResolutions(structure.get());

  return structure;
}

std::vector<FrameDependencyTemplate>
RtpDependencyDescriptorReader::ReadTemplateLayers() {
  ...
  } while (next_layer_idc != kNoMoreTemplates);
...
void RtpDependencyDescriptorReader::ReadTemplateFdiffs(
    FrameDependencyStructure* structure) {
  for (FrameDependencyTemplate& current_template : structure->templates) {
    uint32_t fdiff_follows = ReadBits(1);
    while (fdiff_follows) {
...
void RtpDependencyDescriptorReader::ReadExtendedFields() {
  ...
  if (template_dependency_structure_present_flag) {
    descriptor_->attached_structure = ReadTemplateDependencyStructure();
    RTC_DCHECK(descriptor_->attached_structure);
  } else {
    descriptor_->attached_structure = nullptr;
  }
}
...
void RtpDependencyDescriptorReader::ReadFrameDependencyDefinition() {
  ...
  if (custom_dtis_flag_)
    ReadFrameDtis(&descriptor_->frame_dependencies);
  if (custom_fdiffs_flag_)
    ReadFrameFdiffs(&descriptor_->frame_dependencies);
  if (custom_chains_flag_)
    ReadFrameChains(&descriptor_->frame_dependencies);
...
void RtpDependencyDescriptorReader::ReadFrameDtis(
  ...
  for (int i = 0; i < structure_->num_decode_targets; ++i) {
...
void RtpDependencyDescriptorReader::ReadFrameChains(
  ...
  for (int i = 0; i < structure_->num_chains; ++i) {
    frame->chain_diffs[i] = ReadBits(8);
  }
}
