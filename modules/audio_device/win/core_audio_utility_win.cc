...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client,
  ...
  double device_period_in_seconds =
      static_cast<double>(
          ReferenceTimeToTimeDelta(default_period).ToMilliseconds()) /
      1000.0L;
...
HRESULT CoreAudioUtility::GetPreferredAudioParameters(
  ...
  HRESULT hr = GetPreferredAudioParametersInternal(client.Get(), params);

  return hr;
}

HRESULT CoreAudioUtility::SharedModeInitialize(IAudioClient* client,
                                               const WAVEFORMATPCMEX* format,
                                               HANDLE event_handle,
  ...
  *endpoint_buffer_size = buffer_size_in_frames;
  ...
  RTC_DLOG(INFO) << "stream latency: "
                 << ReferenceTimeToTimeDelta(latency).ToMilliseconds()
                 << " [ms]";
  return error.Error();
}
