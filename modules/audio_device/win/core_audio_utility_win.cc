...
std::string GetDeviceFriendlyNameInternal(IMMDevice* device) {
  ...
  // The PKEY_AudioEngine_DeviceFormat property specifies the device format,
  // which is the format that the user has selected for the stream that flows
  // between the audio engine and the audio endpoint device when the device
  // operates in shared mode.
  // The blob member of the PROPVARIANT structure is a structure of type BLOB
  // that contains two members. Member blob.cbSize is a DWORD that specifies
  // the number of bytes in the format description. Member blob.pBlobData
  // points to a WAVEFORMATEX structure that contains the format description.

  // TODO(henrika): possibly utilize this information...
  // Needs <initguid.h> as well.
  /*
  ScopedPropVariant device_format_pv;
  hr = properties->GetValue(PKEY_AudioEngine_DeviceFormat,
                            device_format_pv.Receive());
  if (FAILED(hr))
    return std::string();
  if (device_format_pv.get().vt == VT_BLOB &&
      device_format_pv.get().blob.cbSize > 0) {
    RTC_DLOG(INFO) << "blob.cbSize: " << device_format_pv.get().blob.cbSize;
    WAVEFORMATEXTENSIBLE* format_ex = reinterpret_cast<WAVEFORMATEXTENSIBLE*>(
        device_format_pv.get().blob.pBlobData);
     RTC_DLOG(INFO) << WaveFormatExToString(format_ex);
  } */
}
...
ComPtr<IAudioClient2> CreateClientInternal2(IMMDevice* audio_device) {
  if (!audio_device)
    return ComPtr<IAudioClient2>();

  ComPtr<IAudioClient2> audio_client;
  _com_error error = audio_device->Activate(
      __uuidof(IAudioClient2), CLSCTX_INPROC_SERVER, NULL, &audio_client);
  if (error.Error() != S_OK) {
...
HRESULT GetPreferredAudioParametersInternal(IAudioClient* client,
  ...
  HRESULT hr = CoreAudioUtility::GetSharedModeMixFormat(client, &mix_format);
  if (FAILED(hr))
    return hr;

  REFERENCE_TIME default_period = 0;
  hr = CoreAudioUtility::GetDevicePeriod(client, AUDCLNT_SHAREMODE_SHARED,
                                         &default_period);
  if (FAILED(hr))
    return hr;
  ...
  double device_period_in_seconds =
      static_cast<double>(
          CoreAudioUtility::ReferenceTimeToTimeDelta(default_period)
              .ToMilliseconds()) /
      1000.0L;
...
std::string CoreAudioUtility::GetDefaultInputDeviceID() {
...
std::string CoreAudioUtility::GetDefaultOutputDeviceID() {
...
std::string CoreAudioUtility::GetCommunicationsInputDeviceID() {
...
std::string CoreAudioUtility::GetCommunicationsOutputDeviceID() {
...
ComPtr<IAudioClient2> CoreAudioUtility::CreateClient2(
    const std::string& device_id,
    EDataFlow data_flow,
    ERole role) {
  RTC_DLOG(INFO) << "CreateClient2";
  ComPtr<IMMDevice> device(CreateDevice(device_id, data_flow, role));
  return CreateClientInternal2(device.Get());
}

HRESULT CoreAudioUtility::SetClientProperties(IAudioClient2* client) {
...
HRESULT CoreAudioUtility::GetSharedModeMixFormat(IAudioClient* client,
  ...
  RTC_DLOG(INFO) << CoreAudioUtility::WaveFormatExToString(format);
...
bool CoreAudioUtility::IsFormatSupported(IAudioClient* client,
  ...
  if ((error.Error() == S_OK) && (closest_match == nullptr)) {
    ...
    RTC_LOG(INFO) << CoreAudioUtility::WaveFormatExToString(closest_match);
...
HRESULT CoreAudioUtility::GetPreferredAudioParameters(IAudioClient* client,
                                                      AudioParameters* params) {
  RTC_DCHECK(client);
  return GetPreferredAudioParametersInternal(client, params);
}

HRESULT CoreAudioUtility::SharedModeInitialize(
    IAudioClient* client,
    const WAVEFORMATEXTENSIBLE* format,
    HANDLE event_handle,
  ...
  *endpoint_buffer_size = buffer_size_in_frames;
  ...
  RTC_DLOG(INFO)
      << "stream latency: "
      << CoreAudioUtility::ReferenceTimeToTimeDelta(latency).ToMilliseconds()
      << " [ms]";
  return error.Error();
}

ComPtr<IAudioRenderClient> CoreAudioUtility::CreateRenderClient(
    IAudioClient* client) {
...
ComPtr<IAudioCaptureClient> CoreAudioUtility::CreateCaptureClient(
    IAudioClient* client) {
...
ComPtr<IAudioClock> CoreAudioUtility::CreateAudioClock(IAudioClient* client) {
...
bool CoreAudioUtility::FillRenderEndpointBufferWithSilence(
    IAudioClient* client,
...
std::string CoreAudioUtility::WaveFormatExToString(
    const WAVEFORMATEXTENSIBLE* format) {
...
rtc::TimeDelta CoreAudioUtility::ReferenceTimeToTimeDelta(REFERENCE_TIME time) {
  // Each unit of reference time is 100 nanoseconds <=> 0.1 microsecond.
  return rtc::TimeDelta::FromMicroseconds(0.1 * time + 0.5);
}

std::string CoreAudioUtility::ErrorToString(const _com_error& error) {
  char ss_buf[1024];
  rtc::SimpleStringBuilder ss(ss_buf);
