...
CoreAudioBase::CoreAudioBase(Direction direction,
    ...
    : direction_(direction), observer_(observer), format_() {
...
bool CoreAudioBase::Init() {
  ...
  ComPtr<IAudioClient> audio_client =
      CoreAudioUtility::CreateClient(device_id, GetDataFlow(), role);
  if (!audio_client.Get()) {
    return false;
  }
...
void CoreAudioBase::ThreadRun() {
  ...
  win::ScopedMMCSSRegistration mmcss_registration(L"Pro Audio");
  ...
  while (streaming && !error) {
    ...
    switch (wait_result) {
      ...
      case WAIT_OBJECT_0 + 1:
        ...
        error = !observer()->OnDataCallback(device_frequency);
        break;
      default:
        error = true;
        break;
