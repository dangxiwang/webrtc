...
void EchoRemoverImpl::ProcessCapture(
  ...
  std::array<float, kFftLengthBy2Plus1> nearend_spectrum;
  if (aec_state_.UsableLinearEstimate()) {
    std::transform(E2.begin(), E2.end(), Y2.begin(), nearend_spectrum.begin(),
                   [](float a, float b) { return std::min(a, b); });
  } else {
    std::copy(Y2.begin(), Y2.end(), nearend_spectrum.begin());
  }
