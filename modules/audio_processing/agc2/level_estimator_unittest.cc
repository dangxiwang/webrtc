...
// Run audio at specified settings through a level controller, and verify that
// the output level falls within the bounds.
void TestLevelEstimator(int sample_rate_hz,
  ...
  agc2::LevelEstimator level_estimator(sample_rate_hz, &apm_data_dumper);
  ...
  for (int i = 0; i < 500; ++i) {
    const auto level =
        level_estimator.ComputeLevel(*vectors_with_float_frame.float_frame());
...
float TimeMsToDecreaseLevel(int sample_rate_hz,
  ...
  const float input_level = DbfsToLinear(input_level_db);
  ...
  float reduced_level_linear =
      DbfsToLinear(input_level_db - level_reduction_db);
  ...
  while (last_level > reduced_level_linear) {
    const auto level_envelope = level_estimator.ComputeLevel(
        *vectors_with_zero_float_frame.float_frame());
...
TEST(AutomaticGainController2LevelEstimator,
  ...
  TestLevelEstimator(20000, num_channels, kInputLevel, kInputLevel * 0.9,
                     kInputLevel * 1.1);
}

TEST(AutomaticGainController2LevelEstimator, TimeToDecreaseForNormalizedLevel) {
  constexpr float kLevelReductionDb = 25;
  constexpr float kExpectedTime = kLevelReductionDb * kDecayMs;

  float time_to_decrease =
      TimeMsToDecreaseLevel(22000, 1, 0, kLevelReductionDb);
