...
// Updates are only allowed for active speech segments and when `rms_error_dbfs`
// is not empty. A speech segment is considered active if at least
...
void MonoInputVolumeController::Process(
  ...
  if (++frames_since_update_input_volume_ >= update_input_volume_wait_frames_) {
    const float speech_ratio =
        ...
        static_cast<float>(std::max(update_input_volume_wait_frames_, 1));
