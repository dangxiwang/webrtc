...
TEST(AutomaticGainController2RmsLevelEstimator, TimeToAdapt) {
  ...
  // It should at most differ by 20% after one 'window size' interval.
  const float kMaxDifferenceDb =
      0.2 * std::abs(kDifferentSpeechRmsDbfs - kInitialSpeechRmsDbfs);
  RunOnConstantLevel(
  ...
  RunOnConstantLevel(
      static_cast<int>(2 * kFullBufferSizeMs / kFrameDurationMs),
      VadWithLevel::LevelAndProbability(
          1.f, kDifferentSpeechRmsDbfs - kInitialSaturationMarginDb,
          kDifferentSpeechRmsDbfs),
      &level_estimator);
