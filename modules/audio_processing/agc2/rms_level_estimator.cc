...
void RmsLevelEstimator::EstimateLevel(
    VadWithLevel::LevelAndProbability vad_data) {
  const float speech_rms_dbfs = FloatS16ToDbfs(vad_data.speech_rms_linear);
  const float speech_peak_dbfs = FloatS16ToDbfs(vad_data.speech_peak_linear);
  RTC_DCHECK_GT(speech_rms_dbfs, -150.f);
  RTC_DCHECK_LT(speech_rms_dbfs, 50.f);
  RTC_DCHECK_GT(speech_peak_dbfs, -150.f);
  RTC_DCHECK_LT(speech_peak_dbfs, 50.f);
  ...
  const float numerator = last_estimate_with_offset_dbfs_ *
                              sum_of_speech_probabilities_ * leak_factor +
                          speech_rms_dbfs * vad_data.speech_probability;
...
float RmsLevelEstimator::LatestLevelEstimate() const {
