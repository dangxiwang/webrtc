...
float RunFixedGainControllerWithConstantInput(FixedGainController* fixed_gc,
  ...
  for (size_t i = 0; i < num_frames; ++i) {
    VectorFloatFrame vectors_with_float_frame(
        1, rtc::CheckedDivExact(sample_rate, 100ul), input_level);
...
TEST(AutomaticGainController2FixedDigital, CreateUseWithoutLimiter) {
  ...
  FixedGainController fixed_gc(kGainToApplyDb, kSampleRate, false);
  VectorFloatFrame vectors_with_float_frame(
      1, rtc::CheckedDivExact(kSampleRate, 100ul), kInputLevelLinear);
...
TEST(AutomaticGainController2FixedDigital, CreateUseWithLimiter) {
  const size_t kSampleRate = 44000;
  FixedGainController fixed_gc(kGainToApplyDb, kSampleRate, true);
  VectorFloatFrame vectors_with_float_frame(
      1, rtc::CheckedDivExact(kSampleRate, 100ul), kInputLevelLinear);
  auto float_frame = *vectors_with_float_frame.float_frame();
  fixed_gc.Process(float_frame);
...
TEST(AutomaticGainController2FixedDigital, CheckSaturationBehaviorWithLimiter) {
  ...
  for (const auto gain : gains_no_saturation) {
    ...
    FixedGainController fixed_gc_no_saturation(gain, kSampleRate, true);
  ...
  for (const auto gain : gains_saturation) {
    ...
    FixedGainController fixed_gc_saturation(gain, kSampleRate, true);

    // Saturation expected.
    SCOPED_TRACE(std::to_string(gain));
    EXPECT_FLOAT_EQ(
        RunFixedGainControllerWithConstantInput(
            &fixed_gc_saturation, input_level, num_frames, kSampleRate),
        32767.f);
  }
}
...
TEST(AutomaticGainController2FixedDigital, GainShouldChangeOnSetGain) {
  ...
  FixedGainController fixed_gc_no_saturation(gain_db_no_change, kSampleRate,
                                             false);
