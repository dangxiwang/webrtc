...
TEST_F(ProbeControllerTest, TotalBitrateProbing) {
  ...
  probe_controller_->SetTotalBitrate(1 * kMbpsMultiplier);
}

TEST_F(ProbeControllerTest, TotalBitrateNoProbing) {
  probe_controller_->SetBitrates(kMinBitrateBps, 1 * kMbpsMultiplier,
                                 2 * kMbpsMultiplier);

  EXPECT_CALL(pacer_, CreateProbeCluster(_)).Times(0);
  probe_controller_->SetEstimatedBitrate(500000);
  probe_controller_->SetTotalBitrate(250000);
}
