...
void SendSideBandwidthEstimation::UpdateReceiverEstimate(Timestamp at_time,
                                                         DataRate bandwidth) {
  // TODO(srte): Ensure this at the source and use a DCHECK here instead.
  remote_based_limit_ =
      bandwidth.IsZero() ? DataRate::PlusInfinity() : bandwidth;
...
void SendSideBandwidthEstimation::UpdateEstimate(Timestamp at_time) {
  ...
  if (last_fraction_loss_ == 0 && IsInStartPhase(at_time)) {
    // TODO(srte):  Replace this with direct usage of the limit, we never use
    // more than the remote based limit anyway.
    if (remote_based_limit_.IsFinite())
      new_bitrate = std::max(remote_based_limit_, new_bitrate);
...
void SendSideBandwidthEstimation::CapBitrateToThresholds(Timestamp at_time,
                                                         DataRate bitrate) {
  if (remote_based_limit_ > DataRate::Zero() && bitrate > remote_based_limit_) {
    bitrate = remote_based_limit_;
  }
  if (delay_based_limit_ > DataRate::Zero() && bitrate > delay_based_limit_) {
