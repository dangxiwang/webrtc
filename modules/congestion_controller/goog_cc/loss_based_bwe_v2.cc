...
void LossBasedBweV2::UpdateBandwidthEstimate() {
  ...
  UpdateCurrentState(/*old_estimate=*/loss_based_result_.bandwidth_estimate,
                     /*new_estimate=*/bounded_bandwidth_estimate);
  loss_based_result_.bandwidth_estimate = bounded_bandwidth_estimate;
}

void LossBasedBweV2::UpdateCurrentState(DataRate old_estimate,
                                        DataRate new_estimate) {
  if (IsEstimateIncreasingWhenLossLimited(old_estimate, new_estimate) &&
      new_estimate < delay_based_estimate_) {
    loss_based_result_.state = LossBasedState::kIncreasing;
  } else if (new_estimate < delay_based_estimate_) {
