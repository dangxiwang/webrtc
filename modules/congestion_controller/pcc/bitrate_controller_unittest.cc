...
TEST(PccBitrateControllerTest, IncreaseRateWhenNoChangesForTestBitrates) {
  PccBitrateController bitrate_controller(
      ...
      kDynamicBoundaryIncrement, kRttGradientCoefficient, kLossCoefficient,
      kThroughputCoefficient, kThroughputPower, kRttGradientThreshold);
  VivaceUtilityFunction utility_function(
      kRttGradientCoefficient, kLossCoefficient, kThroughputCoefficient,
      kThroughputPower, kRttGradientThreshold);
  std::vector<MonitorInterval> monitor_block{
      MonitorInterval(kTargetSendingRate * (1 + kEpsilon), kStartTime,
                      kIntervalDuration),
      MonitorInterval(kTargetSendingRate * (1 - kEpsilon),
...
TEST(PccBitrateControllerTest, SlowStartMode) {
  ...
  PccBitrateController bitrate_controller(
      ...
      kDynamicBoundaryIncrement, std::move(mock_utility_function));
  std::vector<MonitorInterval> monitor_block{
      MonitorInterval(kTargetSendingRate, kStartTime, kIntervalDuration)};
  // To complete collecting feedback within monitor Block.
  monitor_block[0].OnPacketsFeedback(
      CreatePacketResults({kStartTime + 3 * kIntervalDuration}, {}, {}));
  EXPECT_EQ(
...
TEST(PccBitrateControllerTest, StepSizeIncrease) {
  ...
  PccBitrateController bitrate_controller(
      kInitialConversionFactor, kInitialDynamicBoundary,
      kDynamicBoundaryIncrement, std::move(mock_utility_function));
  // If we are moning in the same direction - the step size should increase.
  EXPECT_EQ(
      bitrate_controller.ComputeRateUpdate(monitor_block, kTargetSendingRate)
          .kbps(),
      kTargetSendingRate.kbps() + kInitialConversionFactor * gradient);
  EXPECT_EQ(
      bitrate_controller.ComputeRateUpdate(monitor_block, kTargetSendingRate)
          .kbps(),
      kTargetSendingRate.kbps() + 2 * kInitialConversionFactor * gradient);
}
