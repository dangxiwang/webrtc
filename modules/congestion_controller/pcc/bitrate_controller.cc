...
PccBitrateController::PccBitrateController(double initial_conversion_factor,
    ...
    : consecutive_rate_adjustments_number_(0),
      ...
      utility_function_(rtc::MakeUnique<VivaceUtilityFunction>(
...
DataRate PccBitrateController::ComputeRateUpdateForSlowStartMode(
    const PccMonitorInterval& monitor_interval,
    DataRate bandwith_estimate) {
  double function_value = utility_function_->Compute(monitor_interval);
  // For the first monitor interval we want to check if the estimate is below
  if (!previous_utility_.has_value()) {
    previous_utility_ = function_value;
    return monitor_interval.GetTargetSendingRate();
  }
  if (function_value > previous_utility_) {
    previous_utility_ = function_value;
    return monitor_interval.GetTargetSendingRate();
  }
...
DataRate PccBitrateController::ComputeRateUpdateForOnlineLearningMode(
    ...
    DataRate bandwith_estimate) {
  double first_value = utility_function_->Compute(block[0]);
  double second_value = utility_function_->Compute(block[1]);
  double first_bitrate_bps = block[0].GetTargetSendingRate().bps();
  double second_bitrate_bps = block[1].GetTargetSendingRate().bps();
  double gradient =
      (first_value - second_value) / (first_bitrate_bps - second_bitrate_bps);
  ...
  return DataRate::bps(
      std::max<double>(0, bandwith_estimate.bps() + rate_change_bps));
}
