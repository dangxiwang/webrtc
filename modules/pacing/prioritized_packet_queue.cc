...
std::array<TimeDelta, PrioritizedPacketQueue::kNumPriorityLevels>
PrioritizedPacketQueue::ToTtlPerPrio(PacketQueueTTL packet_queue_ttl) {
  std::array<TimeDelta, kNumPriorityLevels> ttl_per_prio = {
      TimeDelta::PlusInfinity(), TimeDelta::PlusInfinity(),
      TimeDelta::PlusInfinity(), TimeDelta::PlusInfinity(),
...
void PrioritizedPacketQueue::Push(Timestamp enqueue_time,
  ...
  int prio_level =
      GetPriorityForType(packet_type, prioritize_audio_retransmission_
                                          ...
                                          : absl::nullopt);
  PurgeOldPacketAtPriorityLevel(prio_level, enqueue_time);
...
Timestamp PrioritizedPacketQueue::LeadingPacketEnqueueTimeForRetransmission(
    RtpPacketToSend::OriginalType type) const {
  const int priority_level = GetPriorityForType(
      RtpPacketMediaType::kRetransmission,
      prioritize_audio_retransmission_
          ? absl::optional<RtpPacketToSend::OriginalType>(type)
          : absl::nullopt);
  if (streams_by_prio_[priority_level].empty()) {
    return Timestamp::MinusInfinity();
  }
...
void PrioritizedPacketQueue::MaybeUpdateTopPrioLevel() {
  if (streams_by_prio_[top_active_prio_level_].empty()) {
    ...
    if (size_packets_ == 0) {
      ...
      for (int i = 0; i < kNumPriorityLevels; ++i) {
        PurgeOldPacketAtPriorityLevel(i, last_update_time_);
...
void PrioritizedPacketQueue::PurgeOldPacketAtPriorityLevel(int prio_level,
                                                           Timestamp now) {
  if (prio_level < 0 || prio_level >= kNumPriorityLevels)
    return;
  const TimeDelta& time_to_live = time_to_live_per_prio_[prio_level];
