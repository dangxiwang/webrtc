...
TEST(FrameDelayDeltaKalmanFilterTest,
  ...
  // A 1000-byte frame...
  double frame_size_delta_bytes = 1000.0;
  // ...should take around `1000.0 / (512e3 / 8.0) = 0.015625 ms` to transmit.
  double expected_frame_delay_delta_estimate_ms = 1000.0 / (512e3 / 8.0);

  EXPECT_EQ(filter.GetFrameDelayDeltaEstimateSizeBased(frame_size_delta_bytes),
            expected_frame_delay_delta_estimate_ms);
  EXPECT_EQ(filter.GetFrameDelayDeltaEstimateTotal(frame_size_delta_bytes),
            expected_frame_delay_delta_estimate_ms);
}
...
TEST(FrameDelayDeltaKalmanFilterTest,
  ...
  TimeDelta frame_delay_delta = TimeDelta::Millis(3);
  ...
  double frame_size_delta_bytes = 200;
  ...
  for (int i = 0; i < test_duration_s * framerate_fps; ++i) {
    ...
    filter.PredictAndUpdate(sign * frame_delay_delta,
                            sign * frame_size_delta_bytes, max_frame_size,
                            var_noise);
  }

  // Verify that the filter has converged within a margin of 0.1 ms.
  EXPECT_NEAR(filter.GetFrameDelayDeltaEstimateTotal(frame_size_delta_bytes),
              frame_delay_delta.ms(), 0.1);
}
