...
TEST_F(TestH264SpsPpsTracker, NoNalus) {
  ...
  auto fixed = tracker_.CopyAndFixBitstream(data, &header);

  EXPECT_EQ(fixed.action, H264SpsPpsTracker::kInsert);
  EXPECT_THAT(fixed.bitstream(), ElementsAreArray(data));
}
...
TEST_F(TestH264SpsPpsTracker, FuAFirstPacket) {
  ...
  EXPECT_THAT(fixed.bitstream(), ElementsAreArray(expected));
}
...
TEST_F(TestH264SpsPpsTracker, SpsPpsPacketThenIdrFirstPacket) {
  ...
  auto fixed = tracker_.CopyAndFixBitstream(data, &idr_header);
  EXPECT_EQ(fixed.action, H264SpsPpsTracker::kInsert);
...
TEST_F(TestH264SpsPpsTracker, SpsPpsOutOfBand) {
  ...
  auto fixed = tracker_.CopyAndFixBitstream(kData, &idr_header);
