...
TEST_F(TestVideoReceiver, PaddingOnlyFrames) {
  ...
  header.video.codec = kVideoCodecVP8;
  for (int i = 0; i < 10; ++i) {
    EXPECT_CALL(packet_request_callback_, ResendPackets(_, _)).Times(0);
...
TEST_F(TestVideoReceiver, PaddingOnlyFramesWithLosses) {
  ...
  header.frameType = kEmptyFrame;
  header.video.is_first_packet_in_frame = false;
  header.header.markerBit = false;
...
TEST_F(TestVideoReceiver, PaddingOnlyAndVideo) {
  ...
  header.header.headerLength = 12;
  header.video.codec = kVideoCodecVP8;
  header.video.codecHeader.VP8.pictureId = -1;
  header.video.codecHeader.VP8.tl0PicIdx = -1;
  for (int i = 0; i < 3; ++i) {
