...
VCMPacket::VCMPacket(const uint8_t* ptr,
    ...
    : payloadType(rtpHeader.header.payloadType),
      ...
      codec(kVideoCodecUnknown),
      is_first_packet_in_frame(rtpHeader.video.is_first_packet_in_frame),
      completeNALU(kNaluComplete),
      insertStartCode(false),
      width(rtpHeader.video.width),
      height(rtpHeader.video.height),
      video_header(rtpHeader.video) {
  CopyCodecSpecifics(rtpHeader.video);

  if (markerBit) {
    video_header.rotation = rtpHeader.video.rotation;
  }
  // Playout decisions are made entirely based on first packet in a frame.
  if (is_first_packet_in_frame) {
    video_header.playout_delay = rtpHeader.video.playout_delay;
  } else {
    video_header.playout_delay = {-1, -1};
  }
}
