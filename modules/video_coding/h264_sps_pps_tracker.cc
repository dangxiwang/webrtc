...
H264SpsPpsTracker::FixedBitstream H264SpsPpsTracker::CopyAndFixBitstream(
  ...
  fixed.size = 0;
  if (append_sps_pps) {
    fixed.size += sps->second.size + sizeof(start_code_h264);
    fixed.size += pps->second.size + sizeof(start_code_h264);
  }
  ...
  if (h264_header.packetization_type == kH264StapA) {
    ...
    while (nalu_ptr < bitstream.data() + bitstream.size()) {
      ...
      fixed.size += sizeof(start_code_h264);

      // The first two bytes describe the length of a segment.
      uint16_t segment_length = nalu_ptr[0] << 8 | nalu_ptr[1];
      nalu_ptr += 2;

      fixed.size += segment_length;
    ...
    fixed.size += bitstream.size();
  }

  // Then we copy to the new buffer.
  fixed.data = std::make_unique<uint8_t[]>(fixed.size);
