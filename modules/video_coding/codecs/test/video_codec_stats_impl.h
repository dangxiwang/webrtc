...
class VideoCodecStatsImpl : public VideoCodecStats {
 public:
  ...
  void LogMetrics(const std::vector<Frame>& frames,
                  MetricsLogger* logger,
                  std::string test_case_name) const override;

  // Creates new frame, stores it into frames` cache and returns raw pointer.
  Frame* AddFrame(int frame_num, uint32_t timestamp_rtp, int spatial_idx);

  // Returns raw pointers to requested frame. Returns `nullptr` if frame does
  // not exist.
