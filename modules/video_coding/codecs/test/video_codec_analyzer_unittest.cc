...
TEST(VideoCodecAnalyzerTest, StartEncodeCreatesFrameStats) {
...
TEST(VideoCodecAnalyzerTest, FinishEncodeUpdatesFrameStats) {
  TaskQueueForTest task_queue;
  VideoCodecAnalyzer analyzer(task_queue);
  analyzer.StartEncode(CreateVideoFrame(kTimestamp));
...
TEST(VideoCodecAnalyzerTest, StartDecodeCreatesFrameStats) {
  TaskQueueForTest task_queue;
  VideoCodecAnalyzer analyzer(task_queue);
...
TEST(VideoCodecAnalyzerTest, FinishDecodeUpdatesFrameStats) {
  TaskQueueForTest task_queue;
  VideoCodecAnalyzer analyzer(task_queue);
  analyzer.StartDecode(CreateEncodedImage(kTimestamp, kSpatialIdx));
