...
VideoCodecStats::Psnr CalcPsnr(const I420BufferInterface& ref_buffer,
  ...
  task_queue_.PostTask([this, timestamp_rtp = frame.Timestamp(),
    ...
    if (fs == nullptr) {
      VideoCodecStats::Frame* fs0 =
          stats_.GetFrame(timestamp_rtp, /*spatial_idx=*/0);
      if (fs0 == nullptr) {
        RTC_CHECK_EQ(0, spatial_idx);
        fs = stats_.AddFrame(num_frames_++, timestamp_rtp, spatial_idx);
      } else {
        fs = stats_.AddFrame(fs0->frame_num, timestamp_rtp, spatial_idx);
      }
...
void VideoCodecAnalyzer::FinishDecode(const VideoFrame& frame,
  ...
  if (reference_video_source_ != nullptr) {
    ...
    task_queue_.PostTask([this, decoded_buffer,
      ...
      VideoCodecStats::Psnr psnr = CalcPsnr(*decoded_buffer, *ref_buffer);
