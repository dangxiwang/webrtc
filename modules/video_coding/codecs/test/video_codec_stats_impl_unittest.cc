...
struct VideoCodecStatsAggregationTestParams {
  VideoCodecStats::Filter slicer;
  VideoCodecStats::Stream expected;
};
...
TEST_P(VideoCodecStatsAggregationTest, Aggregate) {
  ...
  std::vector<VideoCodecStats::Frame> frames = stats_.Slice(test_params.slicer);
  VideoCodecStats::Stream ss = stats_.Aggregate(frames);
  EXPECT_EQ(ss.decode_time_us.mean, test_params.expected.decode_time_us.mean);
}
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::ValuesIn(
        {VideoCodecStatsAggregationTestParams(
             {.slicer = {.first_frame = 0, .last_frame = 1},
              .expected = {.decode_time_us = {.mean = (0 + 1 + 10 + 11 + 100 +
                                                       101 + 110 + 111) /
                                                      8}}}),
         // Slicing on frame number
         VideoCodecStatsAggregationTestParams(
             {.slicer = {.first_frame = 1, .last_frame = 1},
              .expected = {.decode_time_us = {.mean = (1 + 11 + 101 + 111) /
                                                      4}}}),
         // Slice on spatial index
         VideoCodecStatsAggregationTestParams(
             {.slicer = {.first_frame = 0, .last_frame = 1, .spatial_idx = 1},
              .expected = {.decode_time_us = {.mean = (10 + 11 + 110 + 111) /
                                                      4}}}),
         // Slice on temporal index
         VideoCodecStatsAggregationTestParams(
             {.slicer = {.first_frame = 0, .last_frame = 1, .temporal_idx = 0},
              .expected = {
                  .decode_time_us = {.mean = (0 + 10 + 100 + 110) / 4}}})}));
