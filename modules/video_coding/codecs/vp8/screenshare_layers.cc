...
Vp8FrameConfig ScreenshareLayers::UpdateLayerConfig(size_t stream_index,
  ...
  switch (layer_state) {
    case TemporalLayerState::kDrop:
      dependency_info = {Dti(""), {kNone, kNone, kNone}};
      break;
    case TemporalLayerState::kTl0:
      // TL0 only references and updates 'last'.
      dependency_info = {Dti("SS"), {kReferenceAndUpdate, kNone, kNone}};
    ...
    case TemporalLayerState::kTl1:
      ...
      dependency_info = {Dti("-R"), {kReference, kReferenceAndUpdate, kNone}};
    ...
    case TemporalLayerState::kTl1Sync:
      ...
      dependency_info = {Dti("-S"), {kReference, kUpdate, kNone}};
      dependency_info.frame_config.packetizer_temporal_idx = 1;
...
void ScreenshareLayers::OnEncodeDone(size_t stream_index,
  ...
  if (number_of_temporal_layers_ == 1) {
    ...
    generic_frame_info.operating_points = Dti("S");
  } else {
    ...
    if (dependency_info) {
      ...
      generic_frame_info.operating_points = dependency_info->operating_points;
    } else {
      RTC_DCHECK(is_keyframe);
      generic_frame_info.operating_points = Dti("SS");
    }
