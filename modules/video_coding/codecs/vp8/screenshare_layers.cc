...
ScreenshareLayers::ScreenshareLayers(int num_temporal_layers, Clock* clock)
    : clock_(clock),
      ...
      checker_(TemporalLayersChecker::CreateTemporalLayersChecker(
          Vp8BufferReferenceControllerType::kBitrateDynamic,
          num_temporal_layers)) {
...
Vp8BufferReferenceController::FrameConfig ScreenshareLayers::UpdateLayerConfig(
    uint32_t timestamp) {
  ...
  if (target_framerate_) {
    ...
    if (encode_framerate_.Rate(now_ms).value_or(0) > *target_framerate_)
      return Vp8BufferReferenceController::FrameConfig(kNone, kNone, kNone);
  ...
  switch (layer_state) {
    case TemporalLayerState::kDrop:
      tl_config =
          Vp8BufferReferenceController::FrameConfig(kNone, kNone, kNone);
      break;
    case TemporalLayerState::kTl0:
      // TL0 only references and updates 'last'.
      tl_config = Vp8BufferReferenceController::FrameConfig(kReferenceAndUpdate,
                                                            kNone, kNone);
    ...
    case TemporalLayerState::kTl1:
      ...
      tl_config = Vp8BufferReferenceController::FrameConfig(
    ...
    case TemporalLayerState::kTl1Sync:
      ...
      tl_config =
          Vp8BufferReferenceController::FrameConfig(kReference, kUpdate, kNone);
      tl_config.packetizer_temporal_idx = 1;
      break;
  }
