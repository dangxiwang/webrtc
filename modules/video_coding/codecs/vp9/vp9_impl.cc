...
int VP9EncoderImpl::GetEncodedLayerFrame(const vpx_codec_cx_pkt* pkt) {
  ...
  if (encoded_image_._length > 0) {
    ...
    int spatial_layer_id = layer_id.spatial_layer_id;
    RTC_DCHECK_LT(spatial_layer_id, num_spatial_layers_);
    encoded_image_._encodedHeight =
        raw_->d_h * svc_params_.scaling_factor_num[spatial_layer_id] /
        svc_params_.scaling_factor_den[spatial_layer_id];
    encoded_image_._encodedWidth =
        raw_->d_w * svc_params_.scaling_factor_num[spatial_layer_id] /
        svc_params_.scaling_factor_den[spatial_layer_id];
