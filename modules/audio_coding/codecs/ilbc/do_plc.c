...
void WebRtcIlbcfix_DoThePlc(
  ...
  int32_t measure, maxMeasure, partial_energy;
  ...
  if (PLI == 1) {
    ...
    */
    ...
    for (i=0; i<iLBCdec_inst->blockl; i++) {
      ...
      /* Compute energy until it is known that more than just noise is needed */
      if (partial_energy < noise_energy_threshold_30dB) {
        partial_energy += PLCresidual[i] * PLCresidual[i];
      }
    }
