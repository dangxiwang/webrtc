...
Operations DecisionLogicNormal::GetDecisionSpecialized(
  ...
  // DTX packets in the buffer (otherwise we should just play out what we have,
  // since we cannot know the exact duration of DTX packets), and if the mute
  // factor is low enough (otherwise the expansion was short enough to not be
  // noticable).
  // Note that the MuteFactor is in Q14, so a value of 16384 corresponds to 1.
  if (postpone_decoding_after_expand_ && prev_mode == kModeExpand &&
      !dtx_future && UnderTargetLevel() && expand.MuteFactor(0) < 16384 / 2) {
    return kExpand;
  }
