...
class DelayManagerTest : public ::testing::Test {
 protected:
  ...
  bool enable_delay_histogram_ = false;
...
void DelayManagerTest::RecreateDelayManager() {
  ...
  if (use_mock_histogram_) {
    ...
    dm_ = absl::make_unique<DelayManager>(
        ...
        enable_rtx_handling_, enable_delay_histogram_, &detector_, &tick_timer_,
        std::move(histogram));
  } else {
...
TEST_F(DelayManagerTest, DelayHistogram) {
  enable_delay_histogram_ = true;
  use_mock_histogram_ = true;
  RecreateDelayManager();
