...
DelayManager::DelayManager(size_t max_packets_in_buffer,
    ...
    : first_packet_received_(false),
      ...
      enable_delay_histogram_(enable_delay_histogram) {
...
std::unique_ptr<DelayManager> DelayManager::Create(
  ...
  int quantile;
  std::unique_ptr<Histogram> histogram;
  bool enable_delay_histogram = false;
  ...
  return absl::make_unique<DelayManager>(
      ...
      enable_rtx_handling, enable_delay_histogram, peak_detector, tick_timer,
      std::move(histogram));
}
...
int DelayManager::Update(uint16_t sequence_number,
  ...
  if (packet_len_ms > 0) {
    ...
    if (IsNewerSequenceNumber(sequence_number, last_seq_no_ + 1)) {
      ...
      // not negative.
      iat_ms -= static_cast<uint16_t>(sequence_number - last_seq_no_ - 1) *
                packet_len_ms;
      iat_ms = std::max(iat_ms, 0);
    } else if (!IsNewerSequenceNumber(sequence_number, last_seq_no_)) {
      iat_ms += static_cast<uint16_t>(last_seq_no_ + 1 - sequence_number) *
                packet_len_ms;
      reordered = true;
    }
...
int DelayManager::SetPacketAudioLength(int length_ms) {
  ...
  if (!enable_delay_histogram_ && frame_length_change_experiment_ &&
      packet_len_ms_ != length_ms && packet_len_ms_ > 0) {
