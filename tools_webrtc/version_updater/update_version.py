...
def _UpdateWebRTCVersion(filename):
    ...
    new_content = re.sub(
        r'WebRTC source stamp [0-9]{4}-[0-9]{2}-[0-9]{2}',
        r'WebRTC source stamp %d-%d-%d' % (d.year, d.month, d.day),
        content,
        flags=re.MULTILINE)
...
def _LocalCommit():
    ...
    d = datetime.datetime.now()
    commit_msg = "Update WebRTC code version (%d-%d-%d).\n\nBugs: None"
    commit_msg = commit_msg % (d.year, d.month, d.day)
