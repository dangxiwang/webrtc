...
def _SendHistogramSet(url, histograms, oauth_token):
  ...
  http = httplib2.Http()
  response, content = http.request(url + '/add_histograms', method='GET',
...
def _WaitForUploadConfirmation(url, oauth_token, upload_token, wait_timeout,
  ...
  Args:
    ...
    wait_timeout: Maximum time in seconds to wait for the confirmation.
    wait_polling_period: Performance Dashboard will be polled every
      wait_polling_period seconds.
  """
