...
// TurnPort destruction may happen asynchronously, but it should only
// take one simulated clock tick.
static constexpr unsigned int kTurnPortDestructionDelay = 1;
...
class TurnPortTest : public testing::Test,
 ...
 public:
  ...
  void TestTurnGracefulReleaseAllocation(ProtocolType protocol_type) {
    ...
    // Now Destroy it.
    turn_port_->ScheduleDestroy();

    EXPECT_TRUE_SIMULATED_WAIT(turn_port_destroyed_,
                               kTurnPortDestructionDelay, fake_clock_);
    turn_port_.release();
  }
