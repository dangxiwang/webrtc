...
TransportDescription* TransportDescriptionFactory::CreateOffer(
  ...
  if (!current_description || options.ice_restart) {
    std::pair<std::string, std::string> ice_credentials = GetIceCredentials();
...
TransportDescriptionFactory::GetIceCredentials() const {
  if (ice_credentials_factory_ != nullptr) {
    return ice_credentials_factory_->GetIceCredentials();
  }
