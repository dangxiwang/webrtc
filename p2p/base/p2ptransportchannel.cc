...
bool P2PTransportChannel::MaybeSwitchSelectedConnection(
  ...
  RTC_LOG(LS_INFO) << "The selected candidate pair may be changed due to "
                   << reason;
...
void P2PTransportChannel::OnPortReady(PortAllocatorSession *session,
  ...
  SortConnectionsAndUpdateState("candidate pairs created at a new port");
}
...
void P2PTransportChannel::OnMessage(rtc::Message *pmsg) {
  switch (pmsg->message_id) {
    case MSG_SORT_AND_UPDATE_STATE: {
      ...
      if (pmsg->pdata) {
        reason_to_sort =
            static_cast<SortCandidatePairRequest*>(pmsg->pdata)->reason_to_sort;
      }
...
void P2PTransportChannel::OnConnectionStateChange(Connection* connection) {
  ...
  RequestSortAndStateUpdate("candidate pair state maybe changed");
}
...
void P2PTransportChannel::OnConnectionDestroyed(Connection* connection) {
  ...
  if (selected_connection_ == connection) {
    ...
    RequestSortAndStateUpdate("candidate pair destroyed");
  } else {
