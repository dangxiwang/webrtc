...
class ClassLoader {
 public:
  explicit ClassLoader(JNIEnv* env) {
    ...
    CHECK_EXCEPTION(env);
    jclass jni_common_class = env->FindClass("org/webrtc/JniCommon");
    CHECK_EXCEPTION(env);
    class_loader_ = env->NewGlobalRef(env->CallStaticObjectMethod(
        jni_common_class,
        env->GetStaticMethodID(jni_common_class, "getClassLoader",
...
jclass GetClass(JNIEnv* env, const char* name) {
  return g_class_loader->FindClass(env, name);
}
