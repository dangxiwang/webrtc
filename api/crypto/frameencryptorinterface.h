...
// the RTPSender it is attached to. It must only be attached to one RTPSender.
// Note:
...
class FrameEncryptorInterface : public rtc::RefCountInterface {
 public:
  ...
  // Set the key that will be used to encrypt each frame.
  virtual void SetKey(rtc::ZeroOnFreeBuffer<uint8_t> key) = 0;
