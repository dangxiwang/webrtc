...
// implementation to encrypt all outgoing frames. The user must also provide
// a FrameDecryptorInterface to be able to decrypt the frames on the receiving
// device. Note this is an additional layer of encryption in addition to the
...
class FrameEncryptorInterface : public rtc::RefCountInterface {
 public:
  ...
  // Attempts to encrypt the encrypted. The layout of the encrypted frame will
  // be [encrypted_data|ctr|mac]. The MAC will be calculated over the encrypted
  // data and the IV.
