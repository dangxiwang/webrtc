...
// implementation for all incoming frames. The user must also provide a
// FrameEncryptorInterface to be able to encrypt the frames being sent out of
// the device. Note this is an additional layer of encyrption in addition to the
// standard DTLS-SRTP mechanism and is not intended to be used without it.
// Note:
...
class FrameDecryptorInterface : public rtc::RefCountInterface {
 public:
  ...
  // Attempts to decrypt the encrypted frame which must be in the format
  // [encrypted_data|ctr|mac]. Decrypt will verify the MAC before decryption and
  // calculate the IV from the counter.
