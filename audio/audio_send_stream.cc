...
AudioSendStream::GetMinMaxBitrateConstraints() const {
  ...
  if (constraints.min < DataRate::Zero()) {
    RTC_LOG(LS_WARNING) << "TargetAudioBitrateConstraints::min is less than 0";
    return absl::nullopt;
  }
  if (constraints.max < DataRate::Zero()) {
    RTC_LOG(LS_WARNING) << "TargetAudioBitrateConstraints::max is less than 0";
    return absl::nullopt;
  }
