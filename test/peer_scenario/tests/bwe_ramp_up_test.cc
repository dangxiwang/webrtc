...
// Test that caller and calee BWE can rampup even if no media packets are sent.
...
TEST_P(BweRampupWithInitialProbeTest, BweRampUpBothDirectionsWithoutMedia) {
  ...
  caller->pc()->RestartBandwidthEstimation({.allow_probe_without_media = true});
  callee->pc()->RestartBandwidthEstimation({.allow_probe_without_media = true});
...
// Test that we can restart bandwidth estimation and send new BWE probes.
// In this test, camera is stopped, and some times later, the app want to get a
// new BWE estimate.
TEST(BweRampupTest, CanRestartBweAfterStopingVideo) {
  PeerScenario s(*::testing::UnitTest::GetInstance()->current_test_info());
  PeerScenarioClient* caller = s.CreateClient({});
  PeerScenarioClient* callee = s.CreateClient({});
