...
void DefaultVideoQualityAnalyzer::OnFrameEncoded(
  ...
  Timestamp now = Now();
  StreamCodecInfo used_encoder{.codec_name = stats.encoder_name,
                               .first_frame_id = frame_id,
                               .last_frame_id = frame_id,
                               .switched_on_at = now,
                               .switched_from_at = now};
...
void DefaultVideoQualityAnalyzer::OnFrameDecoded(
  ...
  Timestamp now = Now();
  StreamCodecInfo used_decoder{.codec_name = stats.decoder_name,
...
void DefaultVideoQualityAnalyzer::RegisterParticipantInCall(
  ...
  for (auto& key_val : stream_to_sender_) {
    ...
    // and any receiver's peer index and copy from it sender side
    // counters.
    ...
    // then `counters` will be empty and `stream_started_time` will be infinite.
    // In such case empty `counters` are ok, but `stream_started_time` we can
    // initialize to
...
void DefaultVideoQualityAnalyzer::FrameInFlight::OnFrameEncoded(
  ...
  // be called multiple time, in such case we should preserve the value
