...
class SackChunk : public Chunk, public TLVTrait<SackChunkConfig> {
 public:
  ...
  SackChunk(uint32_t cumulative_tsn_ack,
            uint32_t a_rwnd,
  ...
  rtc::ArrayView<const uint32_t> duplicate_tsns() const {
 ...
 private:
  ...
  const uint32_t cumulative_tsn_ack_;
  const uint32_t a_rwnd_;
  std::vector<GapAckBlock> gap_ack_blocks_;
  std::vector<uint32_t> duplicate_tsns_;
};
}  // namespace dcsctp
