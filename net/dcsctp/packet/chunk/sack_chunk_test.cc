  ...
  EXPECT_THAT(chunk.gap_ack_blocks(),
              ElementsAre(SackChunk::GapAckBlock(
                          ...
                          SackChunk::GapAckBlock(
                              ...
                              static_cast<uint16_t>(916312083 - cum_ack_tsn))));
  EXPECT_THAT(chunk.duplicate_tsns(), ElementsAre(916312081));
}

TEST(SackChunkTest, SerializeAndDeserialize) {
  SackChunk chunk(/*cumulative_tsn_ack=*/123, /*a_rwnd=*/456,
                  {SackChunk::GapAckBlock(2, 3)},
                  /*duplicate_tsns=*/{1, 2, 3});
  ...
  EXPECT_THAT(deserialized.duplicate_tsns(), ElementsAre(1, 2, 3));
