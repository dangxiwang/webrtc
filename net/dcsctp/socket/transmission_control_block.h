...
class TransmissionControlBlock : public Context {
 public:
  // Creates a transmission control block from a normal connection sequence.
  static std::unique_ptr<TransmissionControlBlock> Create(
  ...
  // Creates a transmission control block from a handover state.
  static std::unique_ptr<TransmissionControlBlock> CreateFromHandover(
      TimerManager& timer_manager,
      absl::string_view log_prefix,
      const DcSctpOptions& options,
      const Capabilities& capabilities,
