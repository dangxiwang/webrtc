...
struct SendOptions {
  ...
  // by the client and the library will not interpret the value in any way.
...
class DcSctpSocketCallbacks {
 public:
  ...
  // Note that this will trigger at most once per message even if the message
  // was retransmitted due to packet loss.
  //
  ...
  // Will be called when a lifecycle event has reached its end. It will be
  // called when processing of a message is complete, no matter how it
  // completed. Will be called after all other lifecycle events, if any.
  //
