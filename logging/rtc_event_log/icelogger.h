...
class IceEventLog {
 public:
  ...
  // description and log these config events, which are consequently buffered in
  // memory before serialized to the output. This method is intended to be used
  // to inject the description of candidate pairs in batch to increase the
  // likelihood that at least one config event for each candidate pair is
  // serialized to the output and the candidate pair description as the meta
  // data is recorded, given the capacity constraint of the buffer and the
