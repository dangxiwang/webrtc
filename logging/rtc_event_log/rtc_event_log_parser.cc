...
ParsedRtcEventLog::GetIceCandidatePairConfig(size_t index) const {
  ...
  res.type = GetRuntimeIceCandidatePairConfigType(config.type());
  ...
  res.candidate_pair_address_family = GetRuntimeIceCandidatePairAddressFamily(
      config.candidate_pair_address_family());
  return res;
}
...
ParsedRtcEventLog::GetIceCandidatePairEvent(size_t index) const {
  ...
  res.timestamp = GetTimestamp(index);
  RTC_CHECK(event.has_type());
  res.type = GetRuntimeIceCandidatePairEventType(event.type());
