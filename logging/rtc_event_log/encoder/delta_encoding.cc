...
uint64_t UnsignedDelta(uint64_t previous, uint64_t current, uint64_t width) {
  return (current - previous) & MaxUnsignedValueOfBitWidth(width);
}
 ...
 public:
  static bool ValidParameters(uint64_t delta_width_bits,
    ...
    return (1 <= delta_width_bits && delta_width_bits <= 64 &&
            1 <= original_width_bits && original_width_bits <= 64 &&
  ...
  FixedLengthEncodingParameters(uint64_t delta_width_bits,
      ...
      : delta_width_bits_(delta_width_bits),
        ...
        value_mask_(MaxUnsignedValueOfBitWidth(original_width_bits_)) {
  ...
  uint64_t original_width_bits() const { return original_width_bits_; }

  // Masks where only the bits relevant in delta/values are turned on.
  // Note: |original_width_bits| corresponds to value_mask(). Renaming one
  // or the other might be in order.
...
std::string FixedLengthDeltaEncoder::EncodeDeltas(
  ...
  uint64_t max_pos_signed_delta;
  uint64_t min_neg_signed_delta;
  CalculateMinAndMaxDeltas(base, values, original_width_bits,
...
void FixedLengthDeltaEncoder::CalculateMinAndMaxDeltas(
  ...
  for (size_t i = 0; i < values.size(); ++i) {
    const uint64_t forward_delta = UnsignedDelta(prev, values[i], bit_width);
    const uint64_t backward_delta = UnsignedDelta(values[i], prev, bit_width);
...
void FixedLengthDeltaEncoder::EncodeSignedDelta(uint64_t previous,
  ...
  const uint64_t backward_delta =
      UnsignedDelta(current, previous, params_.original_width_bits());
...
std::unique_ptr<FixedLengthDeltaDecoder> FixedLengthDeltaDecoder::Create(
  ...
  if (!FixedLengthEncodingParameters::ValidParameters(
          delta_width_bits, signed_deltas, values_optional,
          original_width_bits)) {
...
uint64_t FixedLengthDeltaDecoder::ApplyDelta(uint64_t base,
  ...
  RTC_DCHECK(!params_.values_optional()) << "Not implemented.";  // Reminder.
  RTC_DCHECK_LE(base,
                MaxUnsignedValueOfBitWidth(params_.original_width_bits()));
