...
class DeltaEncodingSpecificEdgeCasesTest
    : public ::testing::TestWithParam<std::tuple<DeltaSignedness, uint64_t>> {
 public:
...
TEST_P(DeltaEncodingSpecificEdgeCasesTest, ReverseSequence) {
  ...
  const uint64_t base = 0xf82d3 & value_mask;  // Arbitrary.
  const std::vector<uint64_t> values = {(base - 1) & value_mask,
                                        (base - 2) & value_mask,
                                        (base - 3) & value_mask};
