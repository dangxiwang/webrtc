...
const std::vector<std::string> kDefaultSlides{
...
TEST(PCFullStackTest, ForemanCifWithoutPacketLossVp9) {
  ...
  auto fixture = CreateTestFixture(
      ...
      [](PeerConfigurer* alice) {
        ...
        auto frame_generator = test::CreateFromYuvFileFrameGenerator(
            {ClipNameToClipPath("foreman_cif")}, video.width, video.height, 1);
...
TEST(PCFullStackTest, ParisQcifWithoutPacketLoss) {
  ...
  auto fixture = CreateTestFixture(
      ...
      [](PeerConfigurer* alice) {
        ...
        video.stream_label = "alice-video";
        auto frame_generator = test::CreateFromYuvFileFrameGenerator(
            {ClipNameToClipPath("paris_qcif")}, video.width, video.height, 1);
        alice->AddVideoConfig(std::move(video), std::move(frame_generator));
      },
      [](PeerConfigurer* bob) {});
  RunParams run_params(TimeDelta::Seconds(kTestDurationSec));
...
TEST(PCFullStackTest, ConferenceMotionHd2000kbps100msLimitedQueue) {
  ...
  auto fixture = CreateTestFixture(
      ...
      [](PeerConfigurer* alice) {
        ...
        video.stream_label = "alice-video";
        auto frame_generator = test::CreateFromYuvFileFrameGenerator(
            {ClipNameToClipPath("ConferenceMotion_1280_720_50")}, video.width,
            video.height, 1);
        alice->AddVideoConfig(std::move(video), std::move(frame_generator));
      },
      [](PeerConfigurer* bob) {});
...
TEST(PCFullStackTest, ScreenshareSlidesVP8_2TL_NoConferenceMode) {
  ...
  auto fixture = CreateTestFixture(
      ...
      [](PeerConfigurer* alice) {
        ...
        video.stream_label = "alice-video";
        auto frame_generator = test::CreateFromYuvFileFrameGenerator(
            kDefaultSlides, video.width, video.height,
            video.screen_share_config->slide_change_interval.seconds() *
                video.fps);
        alice->AddVideoConfig(std::move(video), std::move(frame_generator));
      },
      [](PeerConfigurer* bob) {});
