...
void VideoStreamEncoder::ReconfigureEncoder() {
  ...
  if (rate_allocator_ && last_encoder_rate_setings_) {
    ...
    last_encoder_rate_setings_->framerate_fps = GetInputFramerateFps();
    SetEncoderRates(
        UpdateBitrateAllocationAndNotifyObserver(*last_encoder_rate_setings_));
  }
...
bool VideoStreamEncoder::EncoderPaused() const {
  ...
  return !last_encoder_rate_setings_ ||
         last_encoder_rate_setings_->encoder_target == DataRate::Zero();
}
...
void VideoStreamEncoder::SetEncoderRates(
  ...
  const bool settings_changes = !last_encoder_rate_setings_ ||
                                rate_settings != *last_encoder_rate_setings_;
  if (settings_changes) {
    last_encoder_rate_setings_ = rate_settings;
  }
...
void VideoStreamEncoder::MaybeEncodeVideoFrame(const VideoFrame& video_frame,
  ...
  if (pending_encoder_reconfiguration_) {
    ...
    if (last_encoder_rate_setings_) {
      ...
      // encoder.
      EncoderRateSettings new_rate_settings = *last_encoder_rate_setings_;
      new_rate_settings.framerate_fps =
          static_cast<uint32_t>(framerate_fps + 0.f);
      SetEncoderRates(
  ...
  if (frame_dropping_enabled && frame_dropper_.DropFrame()) {
    RTC_LOG(LS_VERBOSE)
        ...
        << (last_encoder_rate_setings_
                ? last_encoder_rate_setings_->encoder_target.bps()
                : 0)
