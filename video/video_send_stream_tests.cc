 ...
 private:
  ...
  Action OnSendRtp(const uint8_t* packet, size_t length) override {
    ...
    if (payload_length > 0 && new_packet) {
      ...
      EXPECT_EQ(VideoCodecType::kVideoCodecVP9, parsed.video.codec);
      // Verify common fields for all configurations.
      VerifyCommonHeader(parsed.video.codecHeader.VP9);
      CompareConsecutiveFrames(header, parsed.video);
      // Verify configuration specific settings.
      InspectHeader(parsed.video.codecHeader.VP9);
      ...
      last_header_ = header;
      last_vp9_ = parsed.video.codecHeader.VP9;
    }
    return SEND_PACKET;
  }
