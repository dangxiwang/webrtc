...
class RtpVideoStreamReceiverTest : public testing::Test {
 public:
  ...
  WebRtcRTPHeader GetDefaultPacket() {
    ...
    packet.video.codec = kVideoCodecH264;
    return packet;
  }
  ...
  void AddSps(WebRtcRTPHeader* packet,
    ...
    packet->video.codecHeader.H264
        .nalus[packet->video.codecHeader.H264.nalus_length++] = info;
  }
...
TEST_F(RtpVideoStreamReceiverTest, GenericKeyFrame) {
  ...
  rtp_header.video.is_first_packet_in_frame = true;
  rtp_header.frameType = kVideoFrameKey;
  rtp_header.video.codec = kVideoCodecGeneric;
...
TEST_P(RtpVideoStreamReceiverTestH264, InBandSpsPps) {
  ...
  sps_packet.video.is_first_packet_in_frame = true;
  mock_on_complete_frame_callback_.AppendExpectedBitstream(
  ...
  pps_packet.video.is_first_packet_in_frame = true;
  mock_on_complete_frame_callback_.AppendExpectedBitstream(
      kH264StartCode, sizeof(kH264StartCode));
  ...
  AddIdr(&idr_packet, 1);
  idr_packet.video.is_first_packet_in_frame = true;
...
TEST_P(RtpVideoStreamReceiverTestH264, OutOfBandFmtpSpsPps) {
  ...
  idr_packet.video.is_first_packet_in_frame = true;
  idr_packet.frameType = kVideoFrameKey;
  idr_packet.video.codec = kVideoCodecH264;
...
TEST_F(RtpVideoStreamReceiverTest, PaddingInMediaStream) {
  ...
  header.video.is_first_packet_in_frame = true;
  header.header.sequenceNumber = 2;
  header.header.markerBit = true;
  header.frameType = kVideoFrameKey;
  header.video.codec = kVideoCodecGeneric;
  mock_on_complete_frame_callback_.AppendExpectedBitstream(data.data(),
                                                           data.size());
