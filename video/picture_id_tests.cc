...
class PictureIdObserver : public test::RtpRtcpObserver {
 ...
 private:
  ...
  bool ParsePayload(const uint8_t* packet,
    ...
    switch (codec_type_) {
      case kVideoCodecVP8:
        parsed->picture_id = parsed_payload.video.codecHeader.VP8.pictureId;
        parsed->tl0_pic_idx = parsed_payload.video.codecHeader.VP8.tl0PicIdx;
        parsed->temporal_idx = parsed_payload.video.codecHeader.VP8.temporalIdx;
        break;
      case kVideoCodecVP9:
        parsed->picture_id = parsed_payload.video.codecHeader.VP9.picture_id;
        parsed->tl0_pic_idx = parsed_payload.video.codecHeader.VP9.tl0_pic_idx;
        parsed->temporal_idx =
            parsed_payload.video.codecHeader.VP9.temporal_idx;
        break;
      default:
        RTC_NOTREACHED();
        break;
