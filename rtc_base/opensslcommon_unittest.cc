...
SSL_SESSION* CreateSSLSessionWithFakePeerCertificate(SSL_CTX* ssl_ctx) {
  ...
  X509* ssl_peer_certificate =
     d2i_X509(nullptr, &cert_buffer, checked_cast<long>(cert_buffer_len));  // NOLINT
...
TEST(OpenSSLCommonTest, VerifyPeerCertMatchesHostFailsOnNoSSLObject) {
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(nullptr, "webrtc.org"));
}

TEST(OpenSSLCommonTest, VerifyPeerCertMatchesHostFailsOnEmptyHostname) {
  SSL_CTX* ssl_ctx = SSL_CTX_new(DTLSv1_2_client_method());
  SSL* ssl = SSL_new(ssl_ctx);
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, ""));
  SSL_free(ssl);
...
TEST(OpenSSLCommonTest, VerifyPeerCertMatchesHostFailsOnNoPeerCertificate) {
  ...
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "webrtc.org"));
...
TEST(OpenSSLCommonTest, VerifyPeerCertMatchesHostSucceedsOnCorrectHostname) {
  ...
  EXPECT_TRUE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "www.webrtc.org"));
  EXPECT_TRUE(
      OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "alice.webrtc.org"));
  EXPECT_TRUE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "bob.webrtc.org"));
...
TEST(OpenSSLCommonTest, VerifyPeerCertMatchesHostFailsOnInvalidHostname) {
  ...
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "a.b.webrtc.org"));
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "notwebrtc.org"));
  EXPECT_FALSE(OpenSSLCommon::VerifyPeerCertMatchesHost(ssl, "webrtc.org"));
