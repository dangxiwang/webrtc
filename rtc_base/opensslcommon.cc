...
bool OpenSSLCommon::VerifyPeerCertMatchesHost(SSL* ssl,
...
void OpenSSLCommon::LogCertificates(SSL* ssl, X509* certificate) {
// Logging certificates is extremely verbose. So it is disabled by default.
  ...
  if (cipher_name != nullptr) {
    ...
    RTC_DLOG(LS_ERROR) << "SSL_CIPHER_DESCRIPTION() failed to set cipher_name.";
  }
#endif
}
